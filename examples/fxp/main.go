...
func main() {
	...
	source := ftpclient.New(cfg)
	err = source.Dial(sourceAddr)
	if err != nil {
		panic(err)
	}
	defer source.Quit()
	...
	destination := ftpclient.New(cfg)
	err = destination.Dial(destinationAddr)
	if err != nil {
		panic(err)
	}
	...
	wg.Add(1)
	go func() {
		source.GetResponse(226, tm)
		defer wg.Done()
	}()

	wg.Add(1)
	go func() {
		destination.GetResponse(226, tm)
		defer wg.Done()
	}()
